...
def create_or_update_usuario(sender, instance, created, **kwargs):
    # When a new User is created, try to link to an existing Usuario by email
    # if one exists (to avoid UNIQUE constraint errors). Otherwise create a new perfil.
    if created:
        try:
            perfil = Usuario.objects.get(email=instance.email)
            perfil.user = instance
            # prefer existing nombre if present, otherwise set from username
        ...
        # Actualizar email/nombre si ya existe perfil enlazado
        try:
            ...
            # Si no hay perfil enlazado, intentar encontrar uno por email y enlazarlo
            try:
                perfil = Usuario.objects.get(email=instance.email)
                perfil.user = instance
                perfil.nombre = instance.username
...
class Pedido(models.Model):
    ...
    # Número de pedido secuencial por usuario. Nullable para compatibilidad con datos existentes.
    order_number = models.PositiveIntegerField(null=True, blank=True, help_text='Número de pedido secuencial por usuario')
    ...
    def __str__(self):
        display_num = self.order_number if self.order_number else self.id
        return f"Pedido #{display_num} - {self.usuario.nombre}"
