...
from .forms import ArtistaForm, ProductoForm, UsuarioForm, EmpleadoCreationForm, EmpleadoUpdateForm
...
def register_view(request):
    if request.method == 'POST':
        ...
        if not username or not email or not password or not account_type:
            messages.error(request, 'Todos los campos son obligatorios.', extra_tags='auth')
            return redirect('register')

        if User.objects.filter(username=username).exists():
            messages.error(request, 'El nombre de usuario ya existe.', extra_tags='auth')
            return redirect('register')
        ...
        messages.success(request, 'Cuenta creada correctamente. Por favor inicia sesión.', extra_tags='auth')
...
def agregar_cliente(request):
    if request.method == 'POST':
        ...
        form = UsuarioForm()
    return render(request, 'admin_panel/clientes_agregar.html', {'form': form})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.POST.get('is_staff') == 'on':
                ...
                new_user.save()
            # guardar teléfono en perfil Usuario si viene
            tel = form.cleaned_data.get('tel') if hasattr(form, 'cleaned_data') else request.POST.get('tel')
            try:
                perfil = new_user.usuario
                if tel:
                    perfil.tel = tel
                    perfil.save()
            except Exception:
...
def actualizar_empleado(request, empleado_id):
    ...
    if request.method == 'POST':
        form = EmpleadoUpdateForm(request.POST, instance=user_obj)
        if form.is_valid():
            updated = form.save(commit=False)
            # staff flag
            updated.is_staff = True if request.POST.get('is_staff') == 'on' else False
            updated.save()
            # manage group membership: if is_staff, ensure in 'Empleados' group
            group, _ = Group.objects.get_or_create(name='Empleados')
            if updated.is_staff:
                updated.groups.add(group)
            else:
                updated.groups.remove(group)
            # actualizar teléfono en perfil Usuario
            tel = form.cleaned_data.get('tel')
            try:
                perfil = updated.usuario
                perfil.tel = tel or ''
                perfil.save()
            except Exception:
                pass
        ...
        form = EmpleadoUpdateForm(instance=user_obj)
...
def crear_pedido_publico(request):
    ...
    if cart and cart.items.exists():
        ...
        try:
            with transaction.atomic():
                ...
                # Asignar un número de pedido secuencial por usuario de forma atómica
                usuario_locked = Usuario.objects.select_for_update().get(id=usuario.id)
                last_num = Pedido.objects.filter(usuario=usuario_locked).aggregate(max_order=Max('order_number'))['max_order'] or 0
                next_num = last_num + 1

                pedido = Pedido.objects.create(usuario=usuario_locked, cantidad_producto=cantidad_total, total=total, order_number=next_num)
    ...
    # Crear el pedido con número secuencial por usuario de forma atómica
    try:
        with transaction.atomic():
            usuario_locked = Usuario.objects.select_for_update().get(id=usuario.id)
            last_num = Pedido.objects.filter(usuario=usuario_locked).aggregate(max_order=Max('order_number'))['max_order'] or 0
            next_num = last_num + 1
            pedido = Pedido.objects.create(usuario=usuario_locked, cantidad_producto=cantidad, total=total, order_number=next_num)
    except Exception:
        # Fallback simple create si algo falla (mantener compatibilidad)
...
def editar_perfil(request):
    ...
    if request.method == 'POST':
        ...
        # handle username separately to avoid creating/relinking auth Users
        new_username = request.POST.get('username', '').strip()
        if form.is_valid():
            # validate username uniqueness
            if new_username:
                from django.contrib.auth.models import User as AuthUser
                qs = AuthUser.objects.filter(username__iexact=new_username).exclude(pk=request.user.pk)
                if qs.exists():
                    messages.error(request, 'El nombre de usuario ya está en uso por otra cuenta.')
                else:
                    # update auth user safely (preserve is_staff)
                    try:
                        request.user.username = new_username
                        # update email from profile form cleaned data
    ...
    return render(request, 'perfil_editar.html', {'form': form, 'username': request.user.username})
...
def agregar_pedido(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Asignar order_number de forma atómica
            with transaction.atomic():
                usuario_locked = Usuario.objects.select_for_update().get(id=usuario.id)
                last_num = Pedido.objects.filter(usuario=usuario_locked).aggregate(max_order=Max('order_number'))['max_order'] or 0
                next_num = last_num + 1
...
def ver_detalles_pedidos(request):
    # Build grouped data by Pedido so we can display multiple products per order together
    pedidos_qs = Pedido.objects.select_related('usuario').prefetch_related('detalles__producto__artista').all().order_by('-fecha')
    pedidos_agrupados = []
    for p in pedidos_qs:
        detalles = list(p.detalles.select_related('producto__artista', 'usuario').all())
        if not detalles:
            continue
        total_cantidad = sum(d.cantidad_producto for d in detalles)
        total_sum = sum(float(d.total) for d in detalles)
        pedidos_agrupados.append({
            'pedido': p,
            'detalles': detalles,
            'total_cantidad': total_cantidad,
            'total_sum': total_sum,
            'cliente': getattr(p.usuario, 'nombre', '')
        })

    # Also provide raw detalles for backward compatibility
    detalles = DetallePedido.objects.select_related('pedido', 'usuario', 'producto').all().order_by('-fecha')
    return render(request, 'admin_panel/detalles_pedidos_ver.html', {'detalles': detalles, 'pedidos_agrupados': pedidos_agrupados})
